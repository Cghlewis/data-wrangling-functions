...
Another way you can write this is using the selection helper `matches()` which allows you to list several criteria to match, separated by the *and/or* operator `|`.

```{r}

d2 %>% dplyr::select(matches("id|test"))
