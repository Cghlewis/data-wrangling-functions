...
*Note: `dplyr::if_else()` will evaluate in order, so proceed from most specific to most general. So therefore, it will not write over a previous evaluation even if something meets a new criteria. I'll show an example of that in #2.
...
*Note: `dplyr::case_when()` will evaluate in order, so proceed from most specific to most general. So therefore, it will not write over a previous evaluation even if something meets a new criteria. I'll show an example of that in #2.
