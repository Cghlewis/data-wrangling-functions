...
d2 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(measure_sum = mean(dplyr::c_across(item1:item3), na.rm=TRUE))
...
d2 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(measure_sum = round(mean(c_across(item1:item3), na.rm=TRUE), digits=2))
...
d4 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(toca_sum = mean(dplyr::c_across(tidyselect::contains("toca")), na.rm=TRUE))
...
d4 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(toca_sum = mean(dplyr::c_across(c(tidyselect::contains("toca"), -c(toca2, toca4))), na.rm=TRUE))
