...
* Note: The default for `rowSums()` is to **not** calculate a sum if any NA value exists. If you want to still calculate a sum despite missing values, you can add the argument *na.rm = TRUE*.

```{r}

d2 %>% 
  dplyr::mutate(measure_sum = rowSums(dplyr::across(c(item1, item2, item3))))
...
d10 %>% 
  dplyr::mutate(measure_sum = rowSums(dplyr::across(c(item1, item2, item3)), na.rm = TRUE))
...
d10 %>%
  dplyr::mutate(measure_sum =
           dplyr::case_when(
             dplyr::if_all(item1:item3, ~ is.na(.)) ~ NA,
             .default = base::rowSums(dplyr::across(item1:item3), na.rm = TRUE)))
...
* Note: The default for NA values in your columns is to **not** calculate a mean if any NA value exists. If you want to still calculate a mean despite missing values, you can set the argument to *na.rm = TRUE*.
...
d2 %>% 
  dplyr::mutate(measure_mean = rowMeans(dplyr::across(item1:item3), na.rm = TRUE))
...
d2 %>% 
  dplyr::mutate(measure_mean = round(rowMeans(dplyr::across(item1:item3), na.rm=TRUE), digits= 1))
...
d5 %>% 
  dplyr::mutate(
    toca_mean = 
      dplyr::case_when(
        rowSums(is.na(across(starts_with("toca")))) <= 1 ~ 
          rowMeans(dplyr::across(tidyselect::starts_with("toca")), na.rm=TRUE),
    TRUE ~ NA_real_))
...
d4 %>%
  dplyr::rowwise() %>%
  dplyr::mutate(toca_mean = mean(dplyr::c_across(c(
    tidyselect::contains("toca"),-toca2,-toca4
  )), na.rm = TRUE))
