...
This example is based off of a real-life scenario. Oftentimes when you download "select all" questions from a survey platform, each option is downloaded with a 1 (if selected) and NA (if not selected). There are times when this format makes sense (say for visualizations). However, for analysis purposes we probably want those variables to show 1s (Yes) and 0s (No) instead of 1s and NAs.
