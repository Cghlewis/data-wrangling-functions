...
* Note: You can read `dplyr::if_any()` here as, keep rows where ANY variable (var1:var4) is not equal to "MR" (notice the ! on the outside of the `dplyr::if_all()` function)

* Note: You'll notice here that I used *%in%* rather than *==* to ensure I keep the NA values for var1:var4. See [Filter rows with NA values](https://cghlewis.github.io/data-wrangling-functions/filter-data/filter-rows-na.html) to learn more.
