...
This example is based off of a real-life scenario. Oftentimes when you download "select all" questions from a survey platform, each option is downloaded with a 1 (if selected) and NA (if not selected). However, we often want those variables to show 1s (Yes) and 0s (No) instead of 1s and NAs.
...
d18 %>%
  dplyr::mutate(dplyr::across(tidyselect::all_of(vars), ~replace_na(., 0)))
